...
STATUS
pt3_tc_write_tuner_without_addr(PT3_I2C_BUS *bus, PT3_TC *tc, const __u8 *data, __u32 size)
{
	__u8 buf;
...
STATUS
pt3_tc_set_agc_s(PT3_I2C_BUS *bus, PT3_TC *tc, PT3_TC_AGC agc)
{
	STATUS status;
	__u8 data;

	data = (agc == PT3_TC_AGC_AUTO) ? 0xff : 0x00;
	status = pt3_tc_write(bus, tc, 0x0a, &data, 1);
	if (status)
		return status;
	
	data = agc_data_s[BIT_SHIFT_MASK(tc->tc_addr, 1, 1)];
	data |= (agc == PT3_TC_AGC_AUTO) ? 0x01 : 0x00;
	status = pt3_tc_write(bus, tc, 0x10, &data, 1);
	if (status)
		return status;
	...
	status = write_pskmsrst(bus, tc);

	return status;
}

STATUS
pt3_tc_set_agc_t(PT3_I2C_BUS *bus, PT3_TC *tc, PT3_TC_AGC agc)
{
	STATUS status;
	__u8 data;

	data = (agc == PT3_TC_AGC_AUTO) ? 64 : 0;
	status = pt3_tc_write(bus, tc, 0x25, &data, 1);
	if (status)
		return status;

	data = 0x4c;
	data |= (agc == PT3_TC_AGC_AUTO) ? 0x00 : 0x01;
	status = pt3_tc_write(bus, tc, 0x23, &data, 1);
	if (status)
		return status;
	
	status = write_imsrst(bus, tc);

	return status;
}

STATUS
pt3_tc_set_sleep_s(PT3_I2C_BUS *bus, PT3_TC *tc, int sleep)
{
	STATUS status;
	__u8 buf;

	buf = sleep ? 1 : 0;
	status = pt3_tc_write(bus, tc, 0x17, &buf, 1);

	return status;
}

STATUS
pt3_tc_set_ts_pins_mode_s(PT3_I2C_BUS *bus, PT3_TC *tc, PT3_TS_PINS_MODE *mode)
{
	...
	status = pt3_tc_write(bus, tc, 0x1c, &data[0], 1);
	if (status)
		return status;
	status = pt3_tc_write(bus, tc, 0x1f, &data[1], 1);
	if (status)
		return status;
...
STATUS
pt3_tc_set_ts_pins_mode_t(PT3_I2C_BUS *bus, PT3_TC *tc, PT3_TS_PINS_MODE *mode)
{
	__u32 clock_data, byte, valid;
	__u8 data;
	STATUS status;
	...
	status = pt3_tc_write(bus, tc, 0x1d, &data, 1);

	return status;
}

STATUS
pt3_tc_write_slptim(PT3_I2C_BUS *bus, PT3_TC *tc, int sleep)
{
	STATUS status;
	__u8 data;

	data = 1 << 7 | ((sleep ? 1 :0) <<4);
	status = pt3_tc_write(bus, tc, 0x03, &data, 1);

	return status;
}

PT3_TC *
create_pt3_tc(__u8 tc_addr, __u8 tuner_addr)
{
	PT3_TC *tc;
