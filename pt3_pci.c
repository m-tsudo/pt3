...
check_fpga_version(PT3_DEVICE *dev_conf)
{
	...
	if (dev_conf->version.ptn != 3) {
		PT3_PRINTK(0, KERN_ERR "PT3: PTn needs 3.\n");
		return -1;
	}
	PT3_PRINTK(7, KERN_INFO "PT3: Check PTn is passed. n=%d\n", dev_conf->version.ptn);

	if (dev_conf->version.fpga != 0x04) {
		PT3_PRINTK(0, KERN_ERR "PT3: this FPGA version is not supported. version=0x%x\n",
...
set_frequency(int isdb, PT3_TUNER *tuner, __u32 channel, __s32 offset)
{
	...
	PT3_PRINTK(7, KERN_DEBUG "PT3: set_freq isdb=%d tuner_no=%d channel=%d offset=%d\n",
...
init_tuner(PT3_I2C *i2c, PT3_TUNER *tuner)
{
	...
	pt3_qm_init_reg_param(tuner->qm);
	{
		...
		free_pt3_bus(bus);
		if (status) {
			PT3_PRINTK(7, KERN_DEBUG "PT3: fail init_tuner dummy reset. status=0x%x\n", status);
			return status;
		}
	}

	{
		...
		free_pt3_bus(bus);
		if (status) {
			PT3_PRINTK(7, KERN_DEBUG "PT3: fail init_tuner qm init. status=0x%x\n", status);
			return status;
		}
	}
...
tuner_power_on(PT3_DEVICE *dev_conf, PT3_BUS *bus)
{
	...
	for (i = 0; i < MAX_TUNER; i++) {
		...
		if (status)
			PT3_PRINTK(1, KERN_INFO "PT3: tc_init_s[%d] status=0x%x\n", i, status);
	}
	for (i = 0; i < MAX_TUNER; i++) {
		tuner = &dev_conf->tuner[i];
		status = pt3_tc_init_t(tuner->tc_t, NULL);
		if (status)
			PT3_PRINTK(1, KERN_INFO "PT3: tc_init_t[%d] status=0x%x\n", i, status);
	}

	tuner = &dev_conf->tuner[1];
	status = pt3_tc_set_powers(tuner->tc_t, NULL, 1, 0);
	if (status) {
		PT3_PRINTK(7, KERN_DEBUG "PT3: fail set powers.\n");
		goto last;
	}
	...
	for (i = 0; i < MAX_TUNER; i++) {
		...
		if (status)
			PT3_PRINTK(1, KERN_INFO "PT3: fail set ts pins mode s [%d] status=0x%x\n", i, status);
	}
	for (i = 0; i < MAX_TUNER; i++) {
		tuner = &dev_conf->tuner[i];
		status = pt3_tc_set_ts_pins_mode_t(tuner->tc_t, NULL, &pins);
		if (status)
			PT3_PRINTK(1, KERN_INFO "PT3: fail set ts pins mode t [%d] status=0x%x\n", i, status);
	}
	...
	for (i = 0; i < MAX_TUNER; i++) {
		for (j = 0; j < 10; j++) {
			if (j != 0)
				PT3_PRINTK(0, KERN_INFO "PT3: retry init_tuner\n");
		...
		if (status) {
			PT3_PRINTK(7, KERN_INFO "PT3: fail init_tuner %d status=0x%x\n", i, status);
			goto last;
		}
	}
	...
	if (status) {
		PT3_PRINTK(7, KERN_INFO "PT3: failed inst_addr=0x%x status=0x%x\n",
	...
	if (status) {
		 PT3_PRINTK(7, KERN_INFO "PT3: fail tc_set_powers,\n");
		goto last;
	}
...
init_all_tuner(PT3_DEVICE *dev_conf)
{
	...
	if (!pt3_i2c_is_clean(i2c)) {
		PT3_PRINTK(0, KERN_INFO "PT3: cleanup I2C bus.\n");
	...
	if (status)
		goto last;
	PT3_PRINTK(7, KERN_DEBUG "PT3: tuner_power_on\n");
	...
	for (i = 0; i < MAX_TUNER; i++) {
		for (j = 0; j < PT3_ISDB_MAX; j++) {
			...
			if (status) {
				PT3_PRINTK(0, KERN_DEBUG "PT3: fail set_frequency. status=0x%x\n", status);
			}
...
get_cn_agc(PT3_CHANNEL *channel, __u32 *cn, __u32 *curr_agc, __u32 *max_agc)
{
	...
	PT3_PRINTK(7, KERN_INFO "PT3: cn=0x%x\n", *cn);
	PT3_PRINTK(7, KERN_INFO "PT3: agc=0x%x\n", *curr_agc);
...
SetChannel(PT3_CHANNEL *channel, FREQUENCY *freq)
{
	...
	case PT3_ISDB_S :
		...
		if (status) {
			PT3_PRINTK(1, KERN_ERR "PT3: fail get_tmcc_s status=0x%x\n", status);
			return status;
		}
		PT3_PRINTK(7, KERN_DEBUG "PT3: tmcc_s.id = 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",
		...
		if (status) {
			PT3_PRINTK(1, KERN_ERR "PT3: fail set_tmcc_s status=0x%x\n", status);
			return status;
		}
		for (i = 0; i < 1000; i++) {
			status = get_id_s(channel->tuner, &tsid);
			if (status) {
				PT3_PRINTK(1, KERN_ERR "PT3: fail get_id_s status=0x%x\n", status);
				return status;
			}
			PT3_PRINTK(7, KERN_DEBUG "PT3: tsid=0x%x\n", tsid);
	...
	case PT3_ISDB_T :
		...
		if (status) {
			PT3_PRINTK(1, KERN_ERR "PT3: fail get_tmcc_t status=0x%x\n", status);
			return status;
		}
		// wait for fill buffer
...
pt3_open(struct inode *inode, struct file *file)
{
	...
	for (lp = 0; lp < MAX_PCI_DEVICE; lp++) {
		if (device[lp] == NULL) {
			PT3_PRINTK(1, KERN_DEBUG "PT3: device is not exists\n");
			return -EIO;
		}
			...
			for (lp2 = 0; lp2 < MAX_CHANNEL; lp2++) {
				...
				if (channel->minor == minor) {
					if (channel->valid) {
						...
						PT3_PRINTK(1, KERN_DEBUG "PT3: device is already used.\n");
						return -EIO;
					}
					PT3_PRINTK(7, KERN_DEBUG "PT3: selected tuner_no=%d type=%d\n",
...
pt3_read(struct file *file, char __user *buf, size_t cnt, loff_t * ppos)
{
	...
	if (rcnt < 0) {
		PT3_PRINTK(1, KERN_INFO "PT3: fail copy_to_user.\n");
		return -EFAULT;
	}
...
count_used_bs_tuners(PT3_DEVICE *device)
{
	...
	PT3_PRINTK(1, KERN_INFO "PT3: used bs tuners on %p = %d\n", device, count);
...
pt3_do_ioctl(struct file  *file, unsigned int cmd, unsigned long arg0)
{
	...
	case GET_SIGNAL_STRENGTH:
		...
		if (status)
			PT3_PRINTK(1, KERN_INFO "PT3: fail get signal strength status=0x%x\n", status);
	...
	case LNB_ENABLE:
		...
		if (count <= 1) {
			...
			PT3_PRINTK(1, KERN_INFO "PT3: LNB on %s\n", voltage[lnb_eff]);
		}
		return 0;
	case LNB_DISABLE:
		count = count_used_bs_tuners(channel->ptr);
		if (count <= 1) {
			...
			PT3_PRINTK(1, KERN_INFO "PT3: LNB off\n");
		}
		return 0;
	case GET_STATUS:
	...
	case SET_TEST_MODE_ON:
		...
		PT3_PRINTK(7, KERN_DEBUG "PT3: rebuild dma descriptor.\n");
		status = (1 + channel->dma->real_index) * 12345;
		pt3_dma_set_test_mode(channel->dma, 1, (__u16)status, 0, 0);
		PT3_PRINTK(7, KERN_DEBUG "PT3: set test mode.\n");
		schedule_timeout_interruptible(msecs_to_jiffies(10));	
		pt3_dma_set_enabled(channel->dma, 1);
		schedule_timeout_interruptible(msecs_to_jiffies(10));	
		while (1) {
			status = (int)pt3_dma_get_status(channel->dma);
			PT3_PRINTK(7, KERN_DEBUG "PT3: status = 0x%x\n", status);
			if ((status & 0x01) == 0)
				break;
			if ((status >> 24) != 0x47)
				break;
...
pt3_pci_init_one (struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	pci_set_master(pdev);
	PT3_PRINTK(0, KERN_INFO "PT3: Bus Mastering Enabled.\n");
	...
	if ((class_revision & 0xFF) != 1) {
		PT3_PRINTK(0, KERN_ERR "PT3: Revision %x is not supported\n",
				(class_revision & 0xFF));
		goto out_err_reg;
	}
	PT3_PRINTK(7, KERN_DEBUG "PT3: Revision check passed. revision=0x%x\n", class_revision & 0xff);

	dev_conf = kzalloc(sizeof(PT3_DEVICE), GFP_KERNEL);
	if(!dev_conf){
		PT3_PRINTK(0, KERN_ERR "PT3: out of memory !\n");
		goto out_err_reg;
	}
	PT3_PRINTK(7, KERN_DEBUG "PT3: Allocate PT3_DEVICE.\n");
	...
	if (!rc) {
		...
		PT3_PRINTK(0, KERN_ERR "PT3: DMA MASK ERROR\n");
		goto out_err_fpga;
	}
	...
	if (dev_conf->i2c == NULL) {
		PT3_PRINTK(0, KERN_ERR "PT3: cannot allocate i2c.\n");
		goto out_err_fpga;
	}
	...
	PT3_PRINTK(7, KERN_DEBUG "PT3: Allocate tuners.\n");

	rc = init_all_tuner(dev_conf);
	if (rc) {
		PT3_PRINTK(0, KERN_ERR "PT3: fail init_all_tuner. 0x%x\n", rc);
		goto out_err_i2c;
	}

	for(lp = 0 ; lp < MAX_PCI_DEVICE ; lp++){
		PT3_PRINTK(0, KERN_INFO "PT3: device[%d]=%p\n", lp, device[lp]);
		if(device[lp] == NULL){
			device[lp] = dev_conf ;
	...
	for (lp = 0; lp < MAX_CHANNEL; lp++) {
		...
		if (rc < 0) {
			PT3_PRINTK(0, KERN_ERR "PT3: fail cdev_add.\n");
		}
		...
		if (channel->dma == NULL) {
			PT3_PRINTK(0, KERN_ERR "PT3: fail create dma.\n");
			kfree(channel);
			goto out_err_dma;
		}
		...
		PT3_PRINTK(0, KERN_INFO "PT3: card_number = %d channel=%d\n",
...
pt3_pci_remove_one(struct pci_dev *pdev)
{
	...
	if(dev_conf){
		...
		kfree(dev_conf);
		PT3_PRINTK(0, KERN_DEBUG "PT3: free PT3 DEVICE.\n");
	}
	pci_disable_device(pdev);
